...
// Converte "YYYY-MM-DD" em Date local, sem offset de fuso
...
async function init(user) {
  ...
  // CORREÇÃO: atribuindo handler corretamente
  ...
  $("#filter-form").onsubmit = async (e) => {
    e.preventDefault();
    const start =
      $("#flt-data-start").value.split("/").reverse().join("-") || null;
...
async function renderFestas(uid, filter = {}) {
  ...
  const lista = filter.tipo
    ? festas.filter((f) => prefsById[f.id]?.tipo_evento_id == filter.tipo)
    : festas;
  lista.forEach((f) => cont.appendChild(buildCard(f, prefsById[f.id] || {})));
}

function buildCard(f, p) {
  const d = parseDateOnly(f.data_evento),
...
function openEdit(f, p) {
  ...
  $("#fld-data")._flatpickr.setDate(parseDateOnly(f.data_evento));
...
async function saveFesta(e, user) {
  ...
  let categorias = Array.from(
    document.querySelectorAll("input[name='fld-categoria']:checked:not([disabled])")
  ).map((cb) => cb.value);
