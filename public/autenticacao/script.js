...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnLogin) {
    btnLogin.addEventListener("click", async (e) => {
      ...
      if (!validateEmail(email))
        return showToast("E-mail inválido", { type: "warning" });
      if (!validatePassword(password))
        return showToast("Senha precisa ter mínimo de 6 caracteres", {
          type: "warning",
        });
      ...
      try {
        ...
        /* REDIRECIONA PARA O PAINEL (robusto em qualquer ambiente) */
        setTimeout(() => {
          window.location.replace(
            new URL("/paginas/painel.html", location.origin)
          );
        }, 800);
      } catch (err) {
  ...
  if (btnSignup) {
    btnSignup.addEventListener("click", async (e) => {
      ...
      if (!name)
        return showToast("Informe seu nome", { type: "warning" });
      if (!validateEmail(email))
        return showToast("E-mail inválido", { type: "warning" });
      if (!validatePassword(password))
        return showToast("Senha precisa ter ≥6 caracteres", {
          type: "warning",
        });
      ...
      try {
        ...
        /* Volta para a tela de login (mesma lógica de URL base) */
        setTimeout(() => {
